...
def add_data(request):
    ...
    attendance_count = PicData.objects.filter(username=request.user.username).count()
    attendance_on_duty = PicData.objects.filter(username=request.user.username,is_on_duty=1).count()
    if is_on_duty == "on":
        if not created_date:
            ...
            if not int(total_data):
                ...
                data_count = PicData.objects.filter(username=request.user.username,created_date=created_date).count()
                if data_count:
                    ...
                    # sum_efficiency = 150
                    sum_total_list = []
                    sum_total = PicData.objects.values_list('total_data')
                    for i in sum_total:
                    ...
                    sum_efficiency_list = []
                    sum_efficiency = PicData.objects.values_list('efficiency')
                    ...
                    try:
                        previous_data = PicData.objects.filter(username=request.user.username).filter(created_date__lt=created_date)\
                    ...
                    PicData.objects.create(created_date=created_date, total_data=total_data, spend_time=spend_time, 
        ...
        if not created_date:
            ...
            if data_count:
                ...
                if len(remark) > 20:
                    return HttpResponse(json.dumps({'status':'fail', 'id':'add_remark_error', 'content':'●备注超过20字'}))
                PicData.objects.create(created_date=created_date, total_data=0, spend_time=0, 
...
def edit_data(request):
    ...
    obj = PicData.objects.filter(id=nid).first()
    obj_id = obj.id
    ...
    if is_on_duty == "on":
        if not created_date:
            ...
            if not int(total_data):
                ...
                attendance = float('%.2f' % ((int(attendance_on_duty)+1)/(int(attendance_count)+1)))
                user_score = score(attendance*10, total_data, efficiency, 27, previous_data)
                PicData.objects.filter(id=nid).update(created_date=created_date, total_data=total_data, 
        ...
        if not created_date:
            ...
            if created_date_id != created_date:
                ...
                if data_count:
                    ...
                    if len(remark) > 20:
                        return HttpResponse(json.dumps({'status':'fail', 'id':'edit_remark_error', 'content':'●备注超过20字'}))
            PicData.objects.filter(id=nid).update(created_date=created_date, total_data=0, 
...
def del_data(request):
    ...
    time_delta = request.GET.get('time_delta')
    nid = request.POST.get('data_id')
    PicData.objects.filter(id=nid).delete()
...
def send_util():
    ...
    now_delta = date_day + datetime.timedelta(days=-(int(7)-1))
    path = os.getcwd()
    path_use = path.replace('\\', '/')
    avg_score_list = []
    user_list = []
    date_list = []
    # data = PicData.objects.filter(created_date__gte='2017-07-03').filter(created_date__lte='2017-07-09')
    data = PicData.objects.filter(created_date__gte=now_delta).filter(created_date__lte=date_day)
    ...
    total_data_week_diff = total_data_week - 4200*5
    remarks = ""
    ...
    for d in date_list:
        total_data_dict = {}
        total_data = PicData.objects.filter(created_date=d).aggregate(Sum('total_data'))['total_data__sum']
    ...
    for i in total_data_list:
        if int(list(i.values())[0]) < avg_total_data:
            user = PicData.objects.filter(created_date=list(i.keys())[0]).values('username', 'remark', 'remark_flag')
            user_remark_list += "<strong>" + list(i.keys())[0] + "</strong></br>"
            for u in user:
                if int(u['remark_flag']) != 0:
    ...
    for i in users:
        ...
        if avg_user_score < 70:
            # attendance_count = PicData.objects.filter(username=i['username']).filter(created_date__lte='2017-07-09').count()
            # attendance_on_duty = PicData.objects.filter(username=i['username'],is_on_duty=1).filter(created_date__lte='2017-07-09').count()
            attendance_count = PicData.objects.filter(username=i['username']).filter(created_date__lte=date_list[-1]).count()
            attendance_on_duty = PicData.objects.filter(username=i['username'],is_on_duty=1).filter(created_date__lte=date_list[-1]).count()
            for d in date_list:
                try:
                    attendance = float('%.2f' % (int(attendance_on_duty)/int(attendance_count)))
                    attendance_score_list.append(attendance*10)
                    # previous_data = PicData.objects.filter(username=i['username']).filter(created_date__lte='2017-07-09')\
                    # .aggregate(Sum('total_data'))['total_data__sum']
                    previous_data = PicData.objects.filter(username=i['username']).filter(created_date__lte=date_list[-1])\
                    .aggregate(Sum('total_data'))['total_data__sum']
    ...
    <body>
    本周标注总量：'''+ str(total_data_week) +'''<br/>按合计大连提供3人力标准4200条/天对比，每周标准21000条，本周超出标准总量'''\
    + str(total_data_week_diff) +'''条。<br/>
