    ...
    if request.is_ajax():
        ...
        if request.user.is_superuser:
            ...
            try:
                ...
                date_begin_sql = LabelData.objects.values('created_date').distinct().order_by('created_date').first()['created_date'].strftime("%Y-%m-%d")
                date_end_sql = LabelData.objects.values('created_date').distinct().order_by('created_date').last()['created_date'].strftime("%Y-%m-%d")
                if date_begin == "" and date_end == "":
                    if time_delta:
                        now = datetime.now()
                        now_delta = now + timedelta(days=-(int(time_delta)-1))
                        data = LabelData.objects.filter(created_date__gte=now_delta).filter(created_date__lte=now)\
                        .filter(username=request.user.username).order_by('created_date')
                    else:
                        data = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end_sql)\
                        .filter(username=request.user.username).order_by('created_date')
                elif date_begin == "":
                    data = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end)\
                    .filter(username=request.user.username).order_by('created_date')
                elif date_end == "":
                    data = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end_sql)\
                    .filter(username=request.user.username).order_by('created_date')
                else:
                    data = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end)\
                    .filter(username=request.user.username).order_by('created_date')
                user_list = []
            ...
            try:
                ...
                if date_begin == "" and date_end == "":
                    if time_delta:
                        ...
                        table_data = LabelData.objects.filter(created_date__gte=now_delta).filter(created_date__lte=now)\
                        .order_by('created_date')
                        data_bar = LabelData.objects.filter(created_date__gte=now_delta).filter(created_date__lte=now)
                        date_range.append(now_delta.strftime("%Y-%m-%d") + "-----" + now.strftime("%Y-%m-%d"))
                    else:
                        data = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end_sql)\
                        .order_by('username', 'created_date').values_list("id", "username", "created_date", "total_data", "spend_time", "efficiency")
                        table_data = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end_sql)\
                        .order_by('created_date')
                        data_bar = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end_sql)
                        date_range.append(date_begin_sql + "-----" + date_end_sql)
                elif date_begin == "":
                    data = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end)\
                    .order_by('username', 'created_date').values_list("id", "username", "created_date", "total_data", "spend_time", "efficiency")
                    table_data = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end)\
                    .order_by('created_date')
                    data_bar = LabelData.objects.filter(created_date__gte=date_begin_sql).filter(created_date__lte=date_end)
                    date_range.append(date_begin_sql + "-----" + date_end)
                elif date_end == "":
                    data = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end_sql)\
                    .order_by('username', 'created_date').values_list("id", "username", "created_date", "total_data", "spend_time", "efficiency")
                    table_data = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end_sql)\
                    .order_by('created_date')
                    data_bar = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end_sql)
                    date_range.append(date_begin + "-----" + date_end_sql)
                else:
                    data = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end)\
                    .order_by('username', 'created_date').values_list("id", "username", "created_date", "total_data", "spend_time", "efficiency")
                    table_data = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end)\
                    .order_by('created_date')
                    data_bar = LabelData.objects.filter(created_date__gte=date_begin).filter(created_date__lte=date_end)
        ...
        if request.user.is_superuser:
            try:
                ...
                for date in date_list:
                    r = {}
                    total_data = LabelData.objects.filter(created_date=date).aggregate(Sum('total_data'))['total_data__sum']
                    total_data_list.append(total_data)
                    avg_total_data_list.append(total_data)
                    spend_time = LabelData.objects.filter(created_date=date).aggregate(Sum('spend_time'))['spend_time__sum']
        ...
        if request.user.is_superuser:
            try:
                ...
                for u in users:
                    ...
                    attendance_count = LabelData.objects.filter(username=u).filter(created_date__lte=dates[-1]).count()
                    attendance_on_duty = LabelData.objects.filter(username=u,is_on_duty=1).filter(created_date__lte=dates[-1]).count()
                    for d in dates:
                        try:
                            attendance = float('%.2f' % (int(attendance_on_duty)/int(attendance_count)))
                            attendance_score_list.append(attendance*10)
                            previous_data = LabelData.objects.filter(username=u).filter(created_date__lte=dates[-1])\
...
def save_echarts(request):
    if request.is_ajax():
        ...
        res_total = download_image('"http://127.0.0.1:8000/label_data/display_total_data/?flag=total&date_begin=&date_end=&time_delta=8"###echartsTotalData')
        res_total_efficiency = download_image('"http://127.0.0.1:8000/label_data/display_total_data/?flag=total&date_begin=&date_end=&time_delta=8"###echartsTotalEfficiencyData')
        res_avg_score = download_image('"http://127.0.0.1:8000/label_data/display_score/?flag=score&date_begin=&date_end=&time_delta=8"###echartsAvgScoreData')
        res_super_avg_total = download_image('"http://127.0.0.1:8000/label_data/display_superuser_data/?flag=super&date_begin=&date_end=&time_delta=8"###echartsSuperAvgTotalData')
